...
def _clamp_shift(v: Any) -> float:
    """シフト量を -10.0〜10.0 にクランプして float で返す"""
    try:
        n = float(v)
...
def _lm_to_numeric(lm_val: Any, magnitude: Any = 5) -> float:
    """LM曲線の方向文字列を数値に変換する。left=正（左シフト）、right=負（右シフト）"""
    s = str(lm_val).strip().lower() if lm_val is not None else ""
    mag = max(0.5, _clamp_shift(magnitude))
    if s == "left":
        return mag
    if s == "right":
        return -mag
...
def analyze_macro_impact(news_text: str) -> dict:
    ...
    result["is_shift"] = _clamp_shift(result.get("is_shift", 0))
    result["lm_shift"] = _lm_to_numeric(result.get("lm_shift"), result.get("lm_shift_magnitude", 5))
    result["bp_shift"] = _clamp_shift(result.get("bp_shift", 0))
